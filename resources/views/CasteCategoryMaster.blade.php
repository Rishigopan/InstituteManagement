...
<body>
    ...
    <main id="main" class="main">
        ...
        <div class="modal fade addUpdateModal" id="cast_cat_modal" tabindex="-1" data-bs-backdrop="static"
            data-bs-keyboard="false" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered modal-md">
                <div class="modal-content cntrymodalbg">
                    <div class="modal-header modelhead py-2">
                        <h6 class="modal-title" id="exampleModalLabel">Caste Category</h6>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        <form class="CasteCat AddForm" id="cast_cat" novalidate>
                            {{ csrf_field() }}
                            <div class="row">
                                <div class=" col-12">
                                    <label class="mt-2 mb-1 inputlabel">Name<span style="color:red; font-size:15px">
                                            *</span></label><br>
                                    <input type="text" class="form-control mt-1 inputfield" id="cast_cat_name"
                                        name="CasteCatName" placeholder="Enter Caste Category Name" autofocus required>
                                </div>
                                <div class=" col-12">
                        ...
                        <form class="UpdateCasteCat UpdateForm" id="update_cast_cat" style="display: none;" novalidate>
                            {{ csrf_field() }}
                            <div class="row">
                                <div class=" col-12">
                                    <input type="hidden" id="update_cast_cat_id">
                                    <label class="mt-2 mb-1 inputlabel">Name<span style="color:red; font-size:15px">
                                            *</span></label><br>
                                    <input type="text" class="form-control mt-1 inputfield" id="update_cast_cat_name"
                                        name="UpdateCasteCatName" autofocus required>
                                </div>
                                <div class=" col-12">
                                    <label class="mt-3 mb-1 inputlabel">Remarks </label><br>
        ...
        <div class="modal fade " id="cast_catViewModal" tabindex="-1" data-bs-backdrop="static"
            data-bs-keyboard="false" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered modal-md">
                <div class="modal-content cntrymodalbg">
                    <div class="modal-header modelhead py-2">
                    ...
                    <div class="modal-body">
                        <form class="ViewCasteCat" id="view_cast_cat" style="display: none;" novalidate>
                            {{ csrf_field() }}
                            <div class="row">
                                <div class=" col-12">
                                    <input type="hidden" id="view_cast_cat_id">
                                    <label class="mt-2 mb-1 inputlabel">Name</label><br>
                                    <input disabled class="form-control mt-1 inputfield" id="view_cast_cat_name"
                                        name="ViewCasteCatName" autofocus required>
                                </div>
                                <div class=" col-12">
                                    <label class="mt-3 mb-1 inputlabel">Remarks </label><br>
        ...
        <div class="modal fade delModal" id="delModal" tabindex="-1" data-bs-backdrop="static"
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <h4 class="text-center">Do you want to delete this Caste Category?</h4>
        ...
        <div class="container-fluid px-4 ">
            <h4 class="mt-1 d-flex justify-content-center py-1 contitle">Caste Category </h4>
        </div>
            ...
            <div class="container-fluid mainContents">
                <div class="card card-body main_card mt-2 shadow-lg p-3 mb-2">
                    <div class="main_heading d-flex justify-content-between mb-2 shadow p-2 subheading">
                        ...
                        <div class="">
                            <button class="btn AddBtn px-4" data-bs-toggle="modal" data-bs-target="#cast_cat_modal">+
                                Add</button>
    ...
    <script type="text/javascript">
        ...
        var MasterTable = $('.MasterTable').DataTable({
            ...
            ajax: "{{ route('castecat.Castecatagory') }}",
            columns: [{
                    data: 'DT_RowIndex',
                    name: 'DT_RowIndex'
                },
        ...
        $("#cast_cat").validate({
            ...
            messages: {
                CasteCatName: {
                    required: "Please Enter Caste Category Name",
                    minlength: "atleast 2 characters",
                    maxlength: "maximum 25 characters",
                }
            },
            submitHandler: function(form) {
                var CasteCatName = $('#cast_cat_name').val();
                var CasteCatRemark = $('#remarks').val();
                $.ajax({
                    url: "/api/caste-category",
                    method: "POST",
                    timeout: 0,
                    headers: {
                    ...
                    data: {
                        name: CasteCatName,
                        remarks: CasteCatRemark,
                    },
                    beforeSend: function() {
                        $('.loader').show();
                        $('#cast_cat_modal').modal('hide');
                        $('.mainContents').hide();
        ...
        $('#MasterTable').on('click', '.btn_edit', function() {
            var EditCasteCat = $(this).val();
            console.log(EditCasteCat);

            var settings = {
                "url": "/api/caste-category/" + EditCasteCat + "",
                "method": "GET",
                "timeout": 0,
            };
            ...
            $.ajax(settings).done(function(response) {
                ...
                if (EnquiryTypeedit.success == true) {
                    $('#cast_cat_modal').modal('show');
                    $('#cast_cat').hide();
                    $('#update_cast_cat').show();
                    var CoTypeDataArray = EnquiryTypeedit.castecategories;
                    for (const key in CoTypeDataArray) {
                        var CasteCatName = CoTypeDataArray['name'];
                        var CasteCatRemark = CoTypeDataArray['remarks'];
                        var CasteCatId = CoTypeDataArray['id'];

                    }
                    $("#update_cast_cat_id").val(CasteCatId);
                    $("#update_cast_cat_name").val(CasteCatName);
                    $("#update_remarks").val(CasteCatRemark);
                }
            });
        ...
        $("#update_cast_cat").validate({
            ...
            submitHandler: function(form) {
                var UpdateId = $('#update_cast_cat_id').val();
                var UpdateCasteCatName = $('#update_cast_cat_name').val();
                var UpdateCasteCatRemark = $('#update_remarks').val();
                var UpdatedDy = $('#updated_by').val();


                $.ajax({

                    url: "/api/caste-category/" + UpdateId + "",
                    method: "PUT",
                    timeout: 0,
                    headers: {
                        "Accept": "application/json",
                    ...
                    data: {
                        name: UpdateCasteCatName,
                        remarks: UpdateCasteCatRemark,
        ...
        $('#MasterTable').on('click', '.btn_delete', function() {
            ...
            swal({
                ...
                if (willDelete) {
                    $.ajax({
                        url: "/api/caste-category/" + DeleteEnType,
                        method: "DELETE",
                        timeout: 0,
                        headers: {
                            "Accept": "application/json",
        ...
        $('#MasterTable').on('click', '.btn_view', function() {
            ...
            var settings = {
                "url": "/api/caste-category/" + ViewEnType + "",
                "method": "GET",
                "timeout": 0,
            };
            ...
            $.ajax(settings).done(function(response) {
                ...
                if (Enedit.success == true) {
                    $('#cast_catViewModal').modal('show');
                    $('#view_cast_cat').show();
                    var EnDataArray = Enedit.castecategories;
                    for (const key in EnDataArray) {
                        var CasteCatName = EnDataArray['name'];
                        var CasteCatRemark = EnDataArray['remarks'];
                        var CasteCatId = EnDataArray['id'];

                    }
                    $("#view_cast_cat_id").val(CasteCatId);
                    $("#view_cast_cat_name").val(CasteCatName);
                    $("#view_remarks").val(CasteCatRemark);
                }
            });
        ...
        $(document).ready(function() {
            $('#cast_cat_modal').on('shown.bs.modal', function() {
                $('#cast_cat_name').focus();
            });
        });


        document.getElementById('cast_cat_name').addEventListener('input', function(event) {
        ...
        document.getElementById('update_cast_cat_name').addEventListener('input', function(event) {
            var input = event.target;
            var value = input.value;
            var newValue = value.replace(/[0-9]/g, ''); // Remove any digits from the input value
            if (newValue !== value) {
                input.value = newValue;
            }
        });
