...
<html lang="en">
  ...
  <body>
        ...
        <main id="main" class="main">
            ...
            <div class="modal fade addUpdateModal" id="CourseTypeModal" tabindex="-1" data-bs-backdrop="static"  data-bs-keyboard="false" aria-hidden="true">
                <div class="modal-dialog modal-dialog-centered modal-md">
                    <div class="modal-content cntrymodalbg">
                        <div class="modal-header modelhead py-2">
                            <h6 class="modal-title" id="exampleModalLabel">Course Type</h6>
                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                        </div>
                        <div class="modal-body">
                            <form class="CourseType AddForm" id="course_type" novalidate>
                                {{ csrf_field() }}
                                <div class="row">
                                    <div class=" col-12">
                                        <label class="mt-2 mb-1 inputlabel">Name<span  style="color:red; font-size:15px" > *</span></label><br>
                                        <input type="text" class="form-control mt-1 inputfield" id="course_type_name" name="CourseTypeName" placeholder="Enter Course Type" autofocus required>
                                    </div>                           
                                    <div class=" col-12">
                            ...
                            <form class="UpdateCourseType UpdateForm" id="update_course_type" style="display: none;" novalidate>
                            {{ csrf_field() }}
                                <div class="row">
                                    <div class=" col-12">
                                        <input type="hidden"  id="update_course_type_id" >
                                        <label class="mt-2 mb-1 inputlabel">Name<span  style="color:red; font-size:15px" > *</span></label><br>
                                        <input type="text" class="form-control mt-1 inputfield" id="update_course_type_name" name="UpdateCourseTypeName"  autofocus required>
                                    </div>                           
                                    <div class=" col-12">
                                        <label class="mt-3 mb-1 inputlabel">Remarks </label><br>
            ...
            <div class="modal fade " id="course_typeViewModal" tabindex="-1" data-bs-backdrop="static"               data-bs-keyboard="false" aria-hidden="true">
                <div class="modal-dialog modal-dialog-centered modal-md">
                    <div class="modal-content cntrymodalbg">
                        <div class="modal-header modelhead py-2">
                            <h6 class="modal-title" id="exampleModalLabel">Course Type</h6>
                        ...
                        <div class="modal-body">
                            <form class="ViewCourseType " id="view_course_type" style="display: none;" novalidate>
                            {{ csrf_field() }}
                                <div class="row">
                                    <div class=" col-12">
                                        <input type="hidden"  id="view_course_type_id" >
                                        <label class="mt-2 mb-1 inputlabel">Name</label><br>
                                        <input  disabled class="form-control mt-1 inputfield" id="view_course_type_name" name="ViewCourseName"  autofocus required>
                                    </div>                           
                                    <div class=" col-12">
                                        <label class="mt-3 mb-1 inputlabel">Remarks </label><br>
            ...
            <div class="modal fade delModal" id="delModal" tabindex="-1" data-bs-backdrop="static" aria-labelledby="exampleModalLabel" aria-hidden="true">
                <div class="modal-dialog">
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <h4 class="text-center">Do you want to delete this Course Type ?</h4>
             ...
             <div class="modal fade ResponseModal" id="ResponseModal" tabindex="-1" data-bs-backdrop="static" data-bs-keyboard="false" aria-labelledby="exampleModalLabel" aria-hidden="true">
                ...
                <div class="container-fluid px-4 ">
                    <h4 class="mt-1 d-flex justify-content-center py-1 contitle">Course Type </h4>            
                </div>
                    ...
                    <div class="container-fluid mainContents">
                        <div class="card card-body main_card mt-2 shadow-lg p-3 mb-2">                           
                            <div class="main_heading d-flex justify-content-between mb-2 shadow p-2 subheading">
                                ...
                                <div class="">
                                    <button class="btn AddBtn px-4" data-bs-toggle="modal" data-bs-target="#CourseTypeModal">+  Add</button> 
  ...
  $('.MasterTable thead tr')
        ...
        $("#course_type").validate({
            ...
            submitHandler: function(form) {
                var CourseName = $('#course_type_name').val();
                var CourseRemark = $('#remarks').val();
                $.ajax({
                    url: "/api/coursetype",
                    method: "POST",
                    timeout: 0,
        ...
        $('#MasterTable').on('click', '.btn_edit', function() {
            var EditCourseType = $(this).val();
            console.log(EditCourseType);

            var settings = {
                "url": "/api/course-type/" + EditCourseType + "",
                "method": "GET",
                "timeout": 0,
            };
            ...
            $.ajax(settings).done(function(response) {
                console.log(response);
                var CourseTypeResult = JSON.stringify(response);
                console.log(CourseTypeResult);
                var CourseTypeedit = JSON.parse(CourseTypeResult);
                if (CourseTypeedit.success == true) {
                    $('#CourseTypeModal').modal('show');
                    $('#course_type').hide();
                    $('#update_course_type').show();
                    var CoTypeDataArray = CourseTypeedit.coursetype;
                    for (const key in CoTypeDataArray) {
                        var CourseName = CoTypeDataArray['name'];
                        var CourseRemark = CoTypeDataArray['remarks'];
                        var CourseTypeId = CoTypeDataArray['id'];

                    }
                    $("#update_course_type_id").val(CourseTypeId);
                    $("#update_course_type_name").val(CourseName);
                    $("#update_remarks").val(CourseRemark);
                }
            });
        ...
        $("#update_course_type").validate({
            ...
            submitHandler: function(form) {
                var UpdateId = $('#update_course_type_id').val();
                var UpdateCoTypeName = $('#update_course_type_name').val();
                var UpdateCoTypeRemark = $('#update_remarks').val();
                var UpdatedDy = $('#updated_by').val();


                $.ajax({

                    url: "/api/coursetype/" + UpdateId + "",
                    method: "PUT",
                    timeout: 0,
                    headers: {
                        "Accept": "application/json",
                    ...
                    data: {
                        name: UpdateCoTypeName,
                        remarks: UpdateCoTypeRemark,
        ...
        $('#MasterTable').on('click', '.btn_delete', function() {
            ...
            swal({
                ...
                if (willDelete) {
                    $.ajax({
                        url: "/api/coursetype/" + DeleteEnType,
                        method: "DELETE",
                        timeout: 0,
                        headers: {
                            "Accept": "application/json",
             ...
             $('#MasterTable').on('click', '.btn_view', function () {
                var ViewCoType = $(this).val();
                console.log(ViewCoType);
                var settings = {
                "url": "/api/course-type/"+ViewCoType+"",
                "method": "GET",
                "timeout": 0,
                };
                ...
                var DeptResult = JSON.stringify(response);
                    console.log(DeptResult);
                    var Coedit = JSON.parse(DeptResult);
                    if (Coedit.success == true) {
                        $('#course_typeViewModal').modal('show');                       
                        $('#view_course_type').show();
                        var CoDataArray = Coedit.coursetype;
                        for(const key in CoDataArray){
                            var  CourseType= CoDataArray['name'];
                            var CourseRemark = CoDataArray['remarks'];
                            var CourseId = CoDataArray['id'];

                        }
                        $("#view_course_type_id").val(CourseId);
                        $("#view_course_type_name").val(CourseType);
                        $("#view_remarks").val(CourseRemark);
                    }
                });
...
//validation
document.getElementById('course_type_name').addEventListener('input', function(event) {
...
$(document).ready(function() {
                $('#CourseTypeModal').on('shown.bs.modal', function() {
