        ...
        <div class="wrapper">
            ...
            <div class="container-fluid mainContents">
                <div class="card card-body main_card mt-2 shadow-lg p-3 mb-2">
                    ...
                    <form class="BatchForm AddForm" id="Batch_edit" novalidate>
                        <div class="row">
                            <input type="hidden" id="update_batch_id" value="{{ $getdata->id }}">
                            ...
                            <div class="col-xl-6 col-lg-6 col-12">
                                ...
                                <select class="form-select inputfield " aria-label="Default select example name"
                                    ...
                                    <option hidden class="inputlabel" value=""> Choose CourseProvider</option>
                            ...
                            <div class="col-xl-6 col-lg-6 col-12">
                                ...
                                <input type="text" class="form-control mt-1 inputfield" id="name" name="Name"
                                    placeholder="please Enter BatchName" value="{{ $getdata->batch_name }}"autofocus
                                    required>
                            ...
                            <div class="col-xl-6 col-lg-6 col-12">
                                ...
                                <input type="number" class="form-control Sessions mt-1 inputfield" id="session"
                                    name="Session" placeholder="" value={{ $getdata->session }} autofocus required>
                            ...
                            <div class="col-xl-6 col-lg-6 col-12">
                                ...
                                <input type="number" class="form-control Duration mt-1 inputfield"
                                    id="duration" name="Duration" placeholder="" value="{{ $getdata->duration }}"
                                    autofocus required>
                                ...
                                <select class="form-select inputfield " aria-label="Default select example name"
                                    ...
                                    <option hidden class="inputlabel" value="{{ $getdata->period }}"> Choose Period
                                    </option>
                                        ...
                                        <tbody id="tb">
                                            @foreach ($sessionDetails as $item)
                                                <tr>
                                                    ...
                                                    <td>{{ $item->sessionFrom }}</td>
                                                    <td>{{ $item->sessionTo }}</td>
...
<script type="text/javascript">
    ...
    //view dropdown datas
    var branchdata = {{ $getdata->branch_id }}
    var Acadamicdata = {{ $getdata->academic_year }}
    var courseProviderdata = {{ $getdata->course_provider_id }}
    var CourseData = {{ $getdata->course_name }}
    var BatchTypeData = {{ $getdata->batch_type_id }}
    var SessionDateData ={{$getdata->startdate}}
    $("#branch").val(branchdata);
    $("#academic_year").val(Acadamicdata);
    $("#course_provider").val(courseProviderdata);
    $("#course_name").val(CourseData);
    $("#batch_type").val(BatchTypeData);
    $(".StartDate").val((SessionDateData.slice(0,10)));
    ...
    $(document).on('click', '#btnClose', function () {
                location.replace("{{ url('/BatchTable') }}")
            });

    $("#Batch_edit").validate({
        submitHandler: function(form, e) {
            e.preventDefault();
            var UpdateId = $('#update_batch_id').val();
            var UpdateBranch = $('#branch').val();
            var UpdateAcadamic = $('#academic_year').val();
            var UpdateCourseprovider = $('#course_provider').val();
            var UpdateCourse = $('#course_name').val();
            var UpdateBatchName = $('#name').val();
            var UpdateBatchType = $('#batch_type').val();
            var UpdateBatchNo = $('#batch_no').val();
            var UpdateSession = $('#session').val();
            var UpdateDuration = $('#duration').val();
            var UpdateStartDate = $('#start_date').val();
            var UpdatePeriod = $('#period').val();
            var UpdatSeat = $('#seat').val();
            ...
            $.ajax({
                ...
                data: {
                    "branch_id": UpdateBranch,
                    "academic_year": UpdateAcadamic,
                    "course_provider_id": UpdateCourseprovider,
                    "course_name": UpdateCourse,
                    "batch_name": UpdateBatchName,
                    "batch_no": UpdateBatchNo,
                    "batch_type_id": UpdateBatchType,
                    "seat": UpdatSeat,
                ...
                if (CoTypeUpdateed.success == true) {
                    if (CoTypeUpdateed.code == "0") {
                        ...
                        $('#ResponseText').text("Batch is Already Present");
