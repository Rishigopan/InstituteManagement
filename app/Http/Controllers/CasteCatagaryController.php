...
class CasteCatagaryController extends Controller
{
    public function Castecatagory(Request $request) {

        if ($request->ajax()) {
            $data = CasteCategory::select( 'id','name','remarks')->get();
        ...
        return view('CasteCategoryMaster');      
